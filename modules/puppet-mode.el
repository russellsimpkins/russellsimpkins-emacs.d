...
;; Provides syntax highlighting, indentation, alignment, movement,
;; Imenu and
;; code checking.

;; Syntax highlighting: Fontification upports all of Puppet 3 syntax,
;; including
;; variable expansion in strings.

;; Indentation: Indent expressions automatically.

;; Alignment: Provide alignment rules for common Puppet expressions,
;; and align
...
;; Imenu: Jump to a tag in the current buffer with `imenu' on C-c
;; C-j.  Index
;; variables, resource defaults, classes, nodes, defined types and
;; resource
;; declarations.

;; Code checking: Validate the syntax of the current buffer with
;; `puppet-validate' on C-c C-v.  Lint the current buffer for semantic
;; errors
;; with `puppet-lint' on C-c C-l.  Apply the current buffer with
;; `puppet-apply'
;; on C-c C-c.

;; Flymake: Flymake support is _not_ provided. See Flycheck at
;; http://flycheck.readthedocs.org/en/latest/ for on-the-fly
;; validation and
  ...
  (defconst puppet-rx-constituents
    `((symbol puppet-rx-symbol 0 nil)
      ...
      ;; http://docs.puppetlabs.com/references/stable/metaparameter.html.
            ;; Strictly speaking, this is no meta parameter, but it's
      ;; so common that
  ...
  (save-excursion
    (save-match-data
      (let ((opoint (point))
        ...
        (when apoint
                    ;; This is a bit of a hack and doesn't allow for
          ;; strings.  We really
...
(defun puppet-indent-line ()
  ...
  (beginning-of-line)
  (if (bobp)
            (indent-line-to 0)                ; First line is always
                            ; non-indented
    (let ((not-indented t)
      ...
      (cond
       (array-start
                ;; This line probably starts with an element from an
  ;; array.
                ;; Indent the line to the same indentation as the
   ;; first
        ...
        (save-excursion
          (if (looking-at "^\\s-*],*")
                            ;; If a closing bracket is on a line by
              ;; itself, align it with the
              ;; opening bracket.
              (progn
            ...
            (forward-char 1)
                        ...
                        ;; If the point is at the end of the line, use
            ;; normal indentation.
            ;;
            ;; For example:
            ;; exec { 'foo':
       ...
       ((and (looking-at "^\\s-*},?\\s-*$") block-indent)
                ;; This line contains a closing brace or a closing
        ;; brace followed by a
                ;; comma and we're at the inner block, so we should
        ;; indent it matching
        ;; the indentation of the opening brace of the block.
        (setq cur-indent block-indent))
       (t
                ;; Otherwise, we did not start on a block-ending-only
   ;; line.
        (save-excursion
                    ;; Iterate backwards until we find an indentation
      ;; hint
          (while not-indented
            (forward-line -1)
            (cond
                          ;; Comment lines are ignored unless we're at
             ;; the start of the
             ;; buffer.
             ((eq (puppet-syntax-context) 'comment)
              (if (bobp)
                  (setq not-indented nil)))
                          ...
                          ;; Brace or paren on a line by itself will
             ;; already be indented to
                          ;; the right level, so we can cheat and stop
             ;; there.
             ((looking-at "^\\s-*[\)}]\\s-*")
              (setq cur-indent (current-indentation))
              (setq not-indented nil))

                          ;; Brace (possibly followed by a comma) or
             ;; paren not on a line by
                          ;; itself will be indented one level too
             ;; much, but don't catch
                          ;; cases where the block is started and
             ;; closed on the same line.
             ((looking-at "^[^\n\({]*[\)}],?\\s-*$")
              (setq cur-indent (- (current-indentation) puppet-indent-level))
              (setq not-indented nil))

                          ;; Indent by one level more than the start
             ;; of our block.  We lose
                          ;; if there is more than one block opened
             ;; and closed on the same
                          ;; line but it's still unbalanced; hopefully
             ;; people don't do that.
             ((looking-at "^.*{[^\n}]*$")
              (setq cur-indent (+ (current-indentation) puppet-indent-level))
              (setq not-indented nil))

                          ;; Indent by one level if the line ends with
             ;; an open paren.
             ((looking-at "^.*\(\\s-*$")
              (setq cur-indent (+ (current-indentation) puppet-indent-level))
              (setq not-indented nil))

                          ;; Semicolon ends a block for a resource
             ;; when multiple resources
                          ;; are defined in the same block, but try
             ;; not to get the case of
                          ;; a complete resource on a single line
       ;; wrong.
             ((looking-at "^\\([^'\":\n]\\|\"[^\n\"]*\"\\|'[^\n']*'\\)*;\\s-*$")
              (setq cur-indent (- (current-indentation) puppet-indent-level))
              (setq not-indented nil))

                          ;; Indent an extra level after : since it
             ...
             ((bobp)
              (setq not-indented nil)))))
                ...
                ;; If this line contains only a closing paren, we
        ;; should lose one
...
(defvar puppet-font-lock-keywords
  `(
    ...
    ;; http://docs.puppetlabs.com/puppet/latest/reference/lang_resources.html
        (,(puppet-rx (group (repeat 0 2 "@") ; Virtual and exported
                               ; resources
...
match data from propertization."
  (let* ((beg (match-beginning (or group 1)))
                  ;; Syntax functions can modify the match data, so we
         ;; must preserve it
  ...
  (let ((case-fold-search nil))
    ...
    (funcall
     (syntax-propertize-rules
            ;; Make double colons part of the surrounding symbol.  We
      ;; can't put the
            ;; colon into symbol syntax, because the colon can appear
      ;; as non-symbol
            ;; character as well (e.g. "package { $foo:"), but we want
      ;; the
            ;; double-colon as part of the symbol to make symbol
      ;; navigation move
            ;; across it, and to make stuff like `thing-at-point'
      ;; behave reasonably
      ((rx "::" symbol-start) (0 "_"))
            ;; Mark regular expression literals in proper contexts
      ;; (nodes, cases,
            ;; selectors and match operators) as strings, to make them
      ;; play nicely
            ;; with sexp navigation and SMIE.  Also propertize them
      ;; for use in font
      ;; lock keywords, since we want to apply our own
            ;; puppet-regular-expression-literal face instead of the
      ;; generic string
      ;; face.
      ((puppet-rx (group "/")
                  ...
                  (zero-or-more space)
                  (or ":" "=>"))
              ;; We propertize the body of the regexp literal, not its
       ;; delimiters, to
              ;; make sure that font lock keywords kick in when the
       ;; literal gets moved
              ;; with point on separator.  The separator is
       ;; propertized by syntactic
              ;; font lock (since we marked it as string delimiter),
       ;; so font lock
              ;; keywords will start in the body of the literal.  If
       ;; we'd propertize
              ;; the separator, font lock keywords would miss the
       ;; property
      ...
      ((puppet-rx (or line-start (not (any "\\")))
                  (zero-or-more "\\\\")
                                    ;; We can't use symbol boundaries
                  ;; here, because
                                    ;; `syntax-propertize-rules'
                  ;; applies all rules at the same
                                    ;; time, so the double-colon scope
                  ;; separator isn't yet part of
                  ;; the symbol at this point.
                  (group "$" (or (and "{" variable-name "}") variable-name)))
       (1 (ignore (progn
                    (puppet-syntax-propertize-match 'puppet-expansion)
                                        ;; Propertize all scope
                    ;; operators in the current variable
    ...
    (while (re-search-forward pattern nil 'no-error)
      (let ((entry (cons (match-string 1) (match-beginning 1))))
        (unless (puppet-in-string-or-comment-p (match-beginning 0))
                    ;; Skip this match if it's inside a string or
   ;; comment
          (push entry entries))))
    (nreverse entries)))
  ...
  (let ((case-fold-search nil)
        ...
        (defines (puppet-imenu-collect-entries
                  (puppet-rx (symbol "define")
                             ...
                             (group (symbol resource-name)))))
        resources)
        ;; Resources are a little more complicated since we need to
    ;; extract the type
    ;; and the name
    (goto-char (point-min))
    (while (re-search-forward
                        (puppet-rx (group (repeat 0 2 "@") ; Virtual
                                        ; and exported resources
                                          (symbol resource-name))
                                   (zero-or-more space) "{"
                                                          ;; FIXME:
                                   ;; Support condensed forms
                                   (zero-or-more space)
